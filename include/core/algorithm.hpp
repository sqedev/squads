...
*/
#ifndef __SQUADS_ALGORITHM_H__
#define __SQUADS_ALGORITHM_H__
...
namespace squads {
	...
	SQUADS_TEMPLATE_FULL_DECL_ONE(typename, T)
    inline void  copy_construct(T* mem, const T&& orig) {
	        internal::copy_construct(mem, squads::move(orig), int_to_type <has_trivial_copy<T>::value> ());
	}
	...
	inline T* copy_backward (const T* first, const T* last, T* result) noexcept {
		const size_t nBytes (squads::distance (first, last));
		memmove (advance_ptr(result,-nBytes), first, nBytes);
	}

	SQUADS_TEMPLATE_FULL_DECL_TWO(class, TIter, class, TFn)
	...
	SQUADS_TEMPLATE_FULL_DECL_TWO(class, TIter, class, TPred = squads::less<TIter> )
	constexpr TIter lower_bound (TIter first, TIter last, const TPred& value) {
		TIter mid;
		while (first != last) {

			mid = first + size_t(squads::distance (first,last))/2;
	...
	SQUADS_TEMPLATE_FULL_DECL_THREE(class, TIter, typename, T, class, TPred = squads::less<TIter> )
	...
	inline constexpr bool binary_search (TIter first, TIter last, const TComp& value) {
		TIter found = squads::lower_bound (first, last, value);
	...
	SQUADS_TEMPLATE_FULL_DECL_TWO(class, TIter, typename, T)
	...
	SQUADS_TEMPLATE_FULL_DECL_ONE(typename, TAssignable)
    void swap(TAssignable& a, TAssignable& b) {
        TAssignable tmp = squads::move(a);

        a = squads::move(b);
        b = squads::move(tmp);
	}

	SQUADS_TEMPLATE_FULL_DECL_TWO(typename, TAssignable, size_t, N)
    inline void swap(TAssignable (&x)[N], TAssignable (&y)[N]) {
      for (size_t i = 0; i < N; i++)
			squads::swap(x[i], y[i]);
    }

	SQUADS_TEMPLATE_FULL_DECL_TWO(typename, fIt1, typename, fIt2 )
	...
	void random_shuffle (TRandomAccessIterator first, TRandomAccessIterator last, const TRandFunc& func) {
		for (; first != last; ++ first)
			squads::iter_swap (first, first + (func() % distance (first, last)));
	}
