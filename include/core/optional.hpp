...
#ifndef __SQUADS_OPTIONAL_H__
#define __SQUADS_OPTIONAL_H__
...
namespace squads {
	...
	public:
		...
		basic_optional(const self_type&& other) noexcept
			: m_bHasValue( squads::move(other.m_bHasValue)), m_tValue( squads::move(other.m_tValue)) { }


		template< typename... Args>
		constexpr explicit basic_optional(Args&&... args)
			: m_bHasValue( true), m_tValue( value_type( squads::forward<Args>(args)...))  {}

		template< typename U = value_type >
    	constexpr explicit basic_optional( U && value)
			: m_bHasValue( true), m_tValue( squads::forward<U>( value)) { }
    	 ...
    	 */
    	void reset() noexcept {
        	if ( has_value() & squads::is_class<T>::value)
            	m_tValue->~T();
		...
		void swap( self_type& other) {
			squads::swap(m_bHasValue, other.m_bHasValue);
			squads::swap(m_tValue, other.m_tValue);
    	}
	...
	template< typename T >
	constexpr basic_optional<typename squads::decay<T>::type> make_optional( T && value ) {
		return basic_optional<typename squads::decay<T>::type>( squads::forward<T>( value ) );
	}

	template< typename T, typename...Args >
	constexpr basic_optional<T> make_optional( Args&&... args ) {
		return basic_optional<T>(squads::forward<Args>(args)...);
	}
#if 0
	template< class T >
	struct hash<basic_optional<T>> {
		squads::size_t operator()( const basic_optional<T>& opt ) const noexcept {
