...
#ifndef __SQUADS_HASH_H__
#define __SQUADS_HASH_H__
...
namespace squads {
    using result_type = squads::size_t;
    ...
    struct hash<int8_t>{
		result_type operator () (int8_t n) const noexcept {
			return static_cast<result_type>(n) * SQUADS_THREAD_CONFIG_BASIC_HASHMUL_VAL;
		}
    };
	...
	struct hash_function {
		result_type operator () (T key, size_t maxValue) const noexcept {
			return squads::hash<T>{}(key) % maxValue;
		}
	};
	...
	struct hash_function<const char*> {
		result_type operator () (const char* key, size_t maxValue) const noexcept {
			return squads::hash<const char*>{}(key) % maxValue;
		}
	};
