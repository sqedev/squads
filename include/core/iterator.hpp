...
*/
#ifndef __SQUADS_ITERATOR_H__
#define __SQUADS_ITERATOR_H__
...
namespace squads {
    ...
    namespace internal {
        ...
        distance(TIter first, TIter last, squads::random_access_iterator_tag) {
            return last - first;
        }

        template<typename TIter>
        constexpr typename iterator_traits<TIter>::difference_type
        distance(TIter first, TIter last, squads::input_iterator_tag) {
        ...
        constexpr void advance(TIter& iter, TDistance d, squads::random_access_iterator_tag) {
            iter += d;
        }
        template<typename TIter, typename TDistance = typename iterator_traits<TIter>::difference_type>
        constexpr void advance(TIter& iter, TDistance d, squads::bidirectional_iterator_tag) {
            if (d >= 0) {
                while (d--) ++iter;
            } else {
                while (d++)  --iter;
        ...
        template<typename TIter, typename TDistance = typename iterator_traits<TIter>::difference_type>
        constexpr void advance(TIter& iter, TDistance d, squads::input_iterator_tag) {
            assert(d >= 0);
            while (d--)  ++iter;
        }
    }
	...
	public:
		...
		constexpr difference_type	operator -  (const self_type& i) const {
			return squads::distance (i.m_itter, m_itter); }
	protected:
		iterator m_itter;
	};
